    ...
    try:
        ...
        if all((email, password, username, name)):
            ...
            if DatabaseOperation.load_from_users({"email": email}) == None:
                ...
                return JSONResponse(status_code=401, content={"error_message": "Username is already registered!"})
            return JSONResponse(status_code=401, content={"error_message": "Email is already registered!"})
